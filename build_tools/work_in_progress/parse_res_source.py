...
class Parser(object):
    ...
    def get_cleaned_source(self, to_delete_list):
        assert to_delete_list, "Don't waste time here."
...
def tests():
    ...
    parser = Parser(ex8)
    parser.parse()
    assert parser.get_tree().children[0].name == "\"name:foo\""
    assert parser.get_tree().children[0].block_type == "block"

    parser = Parser(ex9)
    parser.parse()
    assert parser.get_tree().children[0].name == "name"
    assert parser.get_tree().children[0].block_type == "foo"

    # Not supported (or used?)
    #parser = Parser(ex10)
    #parser.parse()
    #assert parser.get_tree().children[0].name == "\"name:foo\""
    #assert parser.get_tree().children[0].block_type == "bar"
...
def main():
    ...
    for file_name in sys.argv[1:]:
        if not "." in file_name and file_name.startswith("ex"):
            source = eval(file_name)
        else:
