...
def main():
    ...
    # Sometimes a language is in the UI and in the accept list but we
    # still want to keep it small. Then it ends up here (for instance
    # the locale "gu").
...
def _process_locale_file(in_file, out, locale_name,
                         ...
                         strip_rare_calendars,
                         strip_duplicate_root_eras):
    # HAS BEEN PORTED TO FILTER_LOCALES_WITH_PARSER.PY
    try:
        _copy_start_of_file(in_file, out, locale_name)
        while True: # Until StopIteration exception.
            # Keep all blocks after "Version" but only a few
        ...
        while True: # Until StopIteration exception.
            ...
            if "{" in line:
                ...
                if block_name == "Languages":
                    ...
                    # Minimal file.
                    # Do not include '%%Parent' line on purpose.
                    keep = block_name == "%%ALIAS"  # TODO: This is broken. Quote chars?
                    _print_or_skip_block(line, in_file, out, keep)
...
def remove_zone_example_cities(in_zone_dir, out_zone_dir):
    # PORTED: AND this is seriously broken when there is no meta
    # block. Also in the sed scripts. See for instance uz_Arab.txt
    # (possibly not used)
