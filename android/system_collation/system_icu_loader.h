...
// Usage is simple:
//
//  1) Call SystemIcuLoader::CreateInstance() to create a new instance.
//
...
public:
  ...
  // Create a new instance. This tries to locate the system ICU library,
  // and the corresponding version suffix that is applied to its entry points,
  // then return a pointer to the corresponding instance on success.
  // will be nullptr if the library cannot be located.
  static SystemIcuLoader* CreateInstance();
};
