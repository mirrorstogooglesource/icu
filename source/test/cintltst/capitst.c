...
void addCollAPITest(TestNode** root)
{
    ...
    // This test crashes when system collation is enabled. The reason for this
    // is that ucol_getTailoredSet() returns a USet* instance allocated by the
    // system library, which is later passed to uset_xxx() functions from
    // the Chromium library, which implements the object differently.
    addTest(root, &TestGetTailoredSet, "tscoll/capitst/TestGetTailoredSet");
#endif  // !UCONFIG_SYSTEM_COLLATION
